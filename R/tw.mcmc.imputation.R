tw.mcmc.imputation <- function( data , iter = 100 , integer = FALSE ){
    ...
    for ( ii in 1:iter ){
        ...
        alpha <- rnorm( N , mean = mean.alpha , sd = sd.alpha )
        # sample beta 
        mean.beta <- colSums( data - outer( alpha , rep( 1 , J ) ) , na.rm=T ) / colSums( ! is.na(data) )
        sd.beta <- sqrt( sig2 / colSums( ! is.na(data) ) )
        beta <- rnorm( J , mean = mean.beta , sd = sd.beta )
        # sample sigma2
        nu <- sum( ! is.na(data)  )
        scale.S <- sum( ( data - outer( alpha , rep(1,J) ) - outer( rep(1,N) , beta ) )^2  , na.rm=T ) / nu
        sig2 <- nu * scale.S / rchisq( 1 , df = nu )
        # sample mu
        mu <- rnorm( 1 , mean = mean( alpha  ) , sd = sqrt( tau2 / N ) ) 
        # sample tau
        nu <- N
        scale.S <- sum( ( alpha - mu )^2 ) / N 
        tau2 <- nu * scale.S / rchisq( 1 , df = nu )
        }    
    # impute missing dataa
    mean.X <- outer( alpha , rep(1,J) ) + outer( rep(1,N) , beta ) 
    sd.X <- sqrt( sig2 )
    data.imp2 <- matrix( rnorm( N*J , mean = mean.X , sd = sd.X ) , ncol=J)
    data.imp <- data
