...
#    dfr$obsindex_upp <- c1[ ind ]
    dfr$obsindex_upp <- c1[ ind ]	
	dfr$obsindex_low <- squeeze( dfr$obsindex_low , c(1,Ny)) 
	dfr$obsindex_upp <- squeeze( dfr$obsindex_upp , c(1,Ny)) 	
...
#    dfr[ dfr$obsindex_upp > Ny  , "obsindex_upp" ] <- Ny
    ...
    for ( dd in 1:donors){
        ydonors[,dd] <- dfr1[ squeeze( dfr0$obsindex_low - dd + 1 ,c(1,Ny) ) , "y"]
        ydonors[,dd+donors] <- dfr1[ squeeze( dfr0$obsindex_upp + dd - 1 ,c(1,Ny) ) , "y"]    
                        }
