...
latent_regression_em_sampling <- function( pv, X, Z=rep(1,length(pv)) )
{
    ...
    res$samp.beta <- miceadds_import_CDM_CDM_rmvnorm( 1, mean=res$est.beta, sigma=res$vcov.beta )
    ...
    samp.gamma <- miceadds_import_CDM_CDM_rmvnorm( 1, mean=stats::coef(mod1), sigma=stats::vcov(mod1) )
